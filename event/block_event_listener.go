...
func getStartBlock(startBlock string) error {
	log.Printf("hello world, %s", startBlock)
	return nil
}

func main() {
	startBlock := flag.String("startBlock", "", "keep start block number if needed")
	flag.Parse()
	err := getStartBlock(*startBlock)
	if err != nil {
		log.Fatalf("err")
	}
